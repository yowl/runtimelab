...
{
    public struct JSCallResultType{}
}
...
namespace WebAssembly.JSInterop
{
    public struct JSCallInfo
    {
        ...
        public Microsoft.JSInterop.JSCallResultType ResultType;
...
namespace System.Private.WebAssembly
{
    ...
    {
        //Uno compatibility
        [DllImport("*", EntryPoint = "nativeaot_wasm_invoke_js")]
        private static extern string InvokeJSInternal(string js, int length, out int exception);

        public static TRes InvokeJS<T0, T1, T2, TRes>(out string exception, ref JSCallInfo callInfo, T0? arg0, T1? arg1, T2? arg2)
        {
            int exInt;
            // TODO-LLVM
            var res = InvokeJSInternal(callInfo.FunctionIdentifier, callInfo.FunctionIdentifier.Length, out exInt);
            exception = exInt.ToString();
            return (TRes)null;
        }
        ...
        [DllImport("*", EntryPoint = "nativeaot_wasm_invoke_js_wasm_invoke_js_unmarshalled")]
