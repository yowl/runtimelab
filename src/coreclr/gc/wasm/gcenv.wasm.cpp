...
// This does not support unmapping of partial allocations either, but it does clear the memory as would mmap
...
{
    printf("VirtualReserveInner requested size %ld with alignment %zx OS_PAGE_SIZE %zx\n", size, alignment, OS_PAGE_SIZE);
...
bool GCToOSInterface::VirtualRelease(void* address, size_t size)
{
    ...
    {
        printf("releasing size not equal to reserved %zu %zu\n", size, blocks[b].size);
        //assert(false);
    }
    blocks[b].used = 0;
    free(address);
